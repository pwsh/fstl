...
inline void GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal, bool restoreBg = false)
{
    ...
    fputc(0xf9, f);
    fputc(0x04, f);
    // fstl patch: with a transparent background each frame must dispose to
    // background (0x09) so previous frames don't show through; otherwise
    // leave prev frame in place (0x05) for delta encoding
    fputc(restoreBg ? 0x09 : 0x05, f);
    fputc(delay & 0xff, f);
...
typedef struct
{
    ...
    uint8_t* oldImage;
    bool firstFrame;
    bool transparent;      // fstl patch: honor source alpha as 1-bit transparency

    uint8_t padding[6];    // make padding explicit
} GifWriter;
...
inline bool GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth = 8, bool dither = false )
{
    ...
    // fstl patch: transparent mode cannot delta-encode (the transparent
    // index is needed for genuinely transparent pixels), so treat every
    // frame as a first frame
    const uint8_t* oldImage = (writer->firstFrame || writer->transparent)? NULL : writer->oldImage;
